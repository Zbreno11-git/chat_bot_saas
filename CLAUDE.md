...
- Runtime env vars live in `web/.env.local` (gitignored): `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SECRET_KEY` (server-only, bypasses RLS — never prefix a secret with `NEXT_PUBLIC_`), `GEMINI_API_KEY`. Optional: `GEMINI_MODEL` (defaults to `gemini-2.5-flash` in `web/lib/llm/gemini.ts`).
